...
public interface WorkingScheduleRepository extends JpaRepository<WorkingSchedule,Integer> {
    ...
    @Query(value = "SELECT EMPLOYEESCHEDULE.WORKINGSCHEDULEID FROM EMPLOYEESCHEDULE INNER JOIN WORKINGSCHEDULE ON EMPLOYEESCHEDULE.WORKINGSCHEDULEID = WORKINGSCHEDULE.WORKINGSCHEDULEID WHERE EMPLOYEESCHEDULE.EMPLOYEEID = (SELECT EMPLOYEE.EMPLOYEEID FROM EMPLOYEE WHERE USERNAME = ?1) AND WORKINGSCHEDULE.DATE = ?2", nativeQuery = true)
     List<Integer> getWorkScheduleByNameAndDate(String name, String date);
}
